...
const intiativeList = () => {
    ...
    const namedoctor=localStorage.getItem('Iddoctor');
    const [intiative, setintiative] = useState();
    const [intiativeLength, setintiativeLength] = useState();

    const idintiative=localStorage.getItem('idintiative');


/*get all intiative and show it in table ________________________*/


useEffect(()=>{

  axios.get(`http://localhost:3030/doctor/getAllintiative`)
    .then(function (response) {
        
      setintiative(response.data)
      setintiativeLength(response.data.length)
    }).catch(function (err) {
      console.log(err);
  });
...
//------------------ validate intiative --------------------------
  ...
  const validateintiative = (id) => {

    
    axios.put(`http://localhost:3030/doctor/validateintiative/${id}`)
    .then(res => {
    console.log(res);

    toastr.success(`intiative Validated Successfully,${new Date()}`, {
...
const deleteintiative = (id)=>{
  var msgConfirmation = window.confirm("Are You Sure Yo want to delete this Order ?");
  if (msgConfirmation) {   
  axios.delete(`http://localhost:3030/doctor/deleteintiativer/${id}`)
  .then(function (response) {
      window.location.reload();
      toastr.success("intiative deleted Successfully", {
        positionClass: "toast-top-left",
        ...
        <aside className="flex-shrink-0 p-4 w-72 h-full ">
          <div className="flex flex-col h-full pt-12 pb-4 rounded-lg neumorphism-shadow bg-white">
            ...
            <div className="flex flex-col items-center justify-center flex-shrink-0 px-4 py-2 mx-4 rounded-lg neumorphism-shadow bg-blue-300">
              ...
              <span href="" target="_blank" className="mt-3 px-4 py-1 rounded-md text-xl font-semibold tracking-wider text-gray-600  focus:outline-none">{namedoctor}</span>
            </div>
            ...
            <nav className="flex-1 max-h-full p-4 mt-6 overflow-y-hidden">
              <ul className="max-h-full p-2 space-y-1 overflow-y-auto divide-y divide-blue-300 neumorphism-shadow">
                {/* Dashboard link */}
                 ...
                 <li>
                  <Link to='/DashboardDoctor' className="flex items-center w-full px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:outline-none focus:ring" data-bs-target="#dashboardCollapse" data-bs-toggle="collapse" aria-expanded="false" aria-controls="dashboardCollapse">
                    <span>
                ...
                <li>
                  <Link className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                    ...
                    <span className="ml-2 font-medium">List intiative</span>
                  </Link>
                </li>
                {/* Delegation link */}
                <li>
                <Link to='/delegation'  className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                      <span>
                        <svg className="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                      </span>
                      <span className="ml-2 font-medium">Space Delegation</span>
                    </Link>
                  </li>
              </ul>
            </nav>
    ...
    <div className="bg-white shadow-md rounded my-6">
      <table className="min-w-max w-full table-auto">
        <thead>
          ...
          <th className="py-3 px-6 text-left">Id event</th>       
            <th className="py-3 px-6 text-center">Price</th>
            <th className="py-3 px-6 text-left">intiativeAddress</th>   
            <th className="py-3 px-6 text-center">Actions</th>
          </tr>
        </thead>
 { intiative && intiative.map(item =>(
          ...
          <td className="py-3 px-6 text-left whitespace-nowrap">
            ...
            <td className="py-3 px-6 text-center">
              ...
              <div className="flex item-center justify-center">               
                <Link onClick={()=>validateintiative(item._id)} className="w-4 mr-2 transform hover:text-yellow-500 hover:scale-110">
                ...
                <Link onClick={()=>deleteintiative(item._id)}  className="w-4 mr-2 transform hover:text-red-500 hover:scale-110">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
