...
const Delegation = () => {
  ...
  // get all Delegation and show it in table
  
    useEffect(()=>{
  
      axios.get(`http://localhost:3030/doctor/getAlldelegation`)
  ...
  const deletedelegation= (id) => {
    var msgConfirmation = window.confirm("Are You Sure Yo want to delete this Event ?");
    if (msgConfirmation) {
    axios.delete(`http://localhost:3030/doctor/deletedelegation/${id}`)
    .then(function (response) {
      console.log('delegation was deleted Succesfully ... ');
    
    })
} 
  
  }
    const getIddelegation = (id) => {
      localStorage.setItem('Iddelegation',id);
      history.push('/editdelegation');
          ...
          <div className="flex flex-col h-full pt-12 pb-4 rounded-lg neumorphism-shadow bg-white">
            ...
            <nav className="flex-1 max-h-full p-4 mt-6 overflow-y-hidden">
              <ul className="max-h-full p-2 space-y-1 overflow-y-auto divide-y divide-blue-300 neumorphism-shadow">
                ...
                <li>
                  <Link to='/intiativeList' className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                    <span>
...
<div className=" flex items-center justify-center font-sans " style={{marginTop:150,marginLeft:250}}>
  ...
  <Link to='/adddelegation' class="bg-blue-300 hover:bg-green-100 text-black font-bold py-2 px-4 rounded-md">Add Delegation</Link>
    <div className="bg-white shadow-md rounded my-6">
      <table className="min-w-max w-full table-auto">
        <thead>
          <tr className="bg-gray-200 text-gray-600  text-sm leading-normal">
            <th className="py-3 px-6 text-left">Delegation name</th>
            <th className="py-3 px-6 text-left">Téléphone</th>
            <th className="py-3 px-6 text-center">Ville</th>
            <th className="py-3 px-6 text-center">Actions</th>
          </tr>
        </thead>
       { delegation && delegation.map(item => (
          ...
          <tr className="border-b border-gray-200 hover:bg-gray-100">
            ...
            <td className="py-3 px-6 text-center">
              ...
              <div className="flex item-center justify-center">               
                <Link onClick={()=>getIddelegation(item._id)}getIddelegation  className="w-4 mr-2 transform hover:text-yellow-500 hover:scale-110">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                  </svg>
                </Link>
                <Link onClick={()=>deletedelegation(item._id)}  className="w-4 mr-2 transform hover:text-red-500 hover:scale-110">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
