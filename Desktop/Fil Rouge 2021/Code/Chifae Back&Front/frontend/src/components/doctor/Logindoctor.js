...
const  Logindoctor = () => {
	...
	const doctor = {login,password};

	axios.post(`http://localhost:3030/doctor/login`,doctor)
		.then(res => {
			console.log(res)
			...
			if(!res.data.message){
			 ...
			 history.push('/DashboardDoctor');
       localStorage.setItem("Iddoctor", login);
			 toastr.success('authenticated SuccessFully', `Welcome ${doctor.login}`, {
...
<div className="min-h-screen bg-gradient-to-r from-green-400 to-blue-500  flex flex-col justify-center sm:py-12">
  <div className="p-10 xs:p-0 mx-auto md:w-full md:max-w-md">
    <h1 className="font-bold text-center text-2xl mb-5">Space Doctor</h1> 
