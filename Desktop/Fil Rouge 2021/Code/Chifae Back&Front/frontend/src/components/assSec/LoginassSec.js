...
const loginassSec=()=> {
	...
	const assSec = {login,password};

	axios.post(`http://localhost:3030/assSec/login`,assSec)
		.then(res => {
      ...
      if(!res.data.message){ 
       ...
       localStorage.setItem("status", status);
        ...
        localStorage.setItem("IdassSec", login);
        // console.log(res.data.id);
        localStorage.setItem("seller",res.data.id);
        history.push('/assSecDashboard');
      ...
      <div className="w-full max-w-lg">
        <div className="leading-loose">
          <form className="max-w-sm m-4 p-10 bg-white bg-opacity-25 rounded shadow-xl"  onSubmit={handleSubmit}>
            <p className="text-black font-medium text-center text-lg font-bold">assSec Login</p>
            <div className="mt-10">
