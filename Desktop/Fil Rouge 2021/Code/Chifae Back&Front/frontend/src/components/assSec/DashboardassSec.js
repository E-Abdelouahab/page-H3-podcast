...
const DashbordassSec = () => {
 
  const history = useHistory();
  const [event, setevent] = useState();
  const idassSec=localStorage.getItem('idAss');
// get all assSec 
...
const logOut =()=>{
    ...
    const assName=localStorage.getItem('idAss');
    axios.get(`http://localhost:3030/assSec/geteventByassSecname/${assName}`)
    .then(function (response) {
          ...
          <aside className="flex-shrink-0 p-4 w-72 h-full">
            <div className="flex flex-col h-full pt-12 pb-4 rounded-lg neumorphism-shadow bg-white">
              ...
              <div className="flex flex-col items-center justify-center flex-shrink-0 px-4 py-2 mx-4 rounded-lg neumorphism-shadow bg-blue-300">
                ...
                <span href="" target="_blank" className="mt-3 px-4 py-1 rounded-md text-xl font-semibold tracking-wider text-gray-600  focus:outline-none">{assName}</span>
              </div>
    ...
    <div className="flex flex-wrap -m-4 text-center">
      ...
      <div className="p-4 md:w-4/4 sm:w-1/2 w-full">
        <div className="border-2 border-gray-200 px-4 py-6 rounded-lg">
          ...
          <h2 className="title-font font-medium text-3xl text-gray-900">{listeventLength}</h2>
      ...
      <div className="p-4 md:w-4/4 sm:w-1/2 w-full">
        <div className="border-2 border-gray-200 px-4 py-6 rounded-lg">
          ...
          <p className="leading-relaxed">Total Donation</p>
        </div>
      </div>
    </div>
  </div>
