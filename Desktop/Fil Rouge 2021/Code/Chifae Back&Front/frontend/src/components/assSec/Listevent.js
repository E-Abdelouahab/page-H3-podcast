...
const  Listevent=()=> {
    ...
    const nameassSec=localStorage.getItem('IdassSec');
    const [listevent, setListevent] = useState();
    const [listeventLength, setlisteventLength] = useState();

    // get all Product and show it in table
    
    const IdassSec=localStorage.getItem('IdassSec');

    
      useEffect(()=>{
    
        axios.get(`http://localhost:3030/assSec/geteventByassSecname/${IdassSec}`)
          .then(function (response) {
              
            setListevent(response.data)
            setlisteventLength(response.data.length)
          }).catch(function (err) {
            console.log(err);
        });
  ...
  const deleteevent = (id)=>{
    ...
    if (msgConfirmation) {   
    axios.delete(`http://localhost:3030/assSec/deleteevent/${id}`)
    .then(function (response) {
  ...
  const checklength = ()=>{
          if(listeventLength > 9){
            toastr.error('You have increase Your Limets pub a pack !! If you want to publisher more ,pls upgrade your acoount')
              ...
              <nav className="flex-1 max-h-full p-4 mt-6 overflow-y-hidden">
                <ul className="max-h-full p-2 space-y-1 overflow-y-auto divide-y divide-blue-300 neumorphism-shadow">
                  {/* Dashboard link */}
                   ...
                   <li>
                    <Link to='/assSecDashboard' className="flex items-center w-full px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:outline-none focus:ring" data-bs-target="#dashboardCollapse" data-bs-toggle="collapse" aria-expanded="false" aria-controls="dashboardCollapse">
                      <span>
                  ...
                  <li>
                    <Link to='buyPack' className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                      ...
                      <span className="ml-2 font-medium">pub Pack</span>
                    </Link>
                  </li>
                  {/* Profile link */}
                  <li>
                    <Link className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                      ...
                      <span className="ml-2 font-medium">List Events</span>
                    </Link>
                  </li>
                </ul>
              </nav>
             ...
             <img src={item.productImg} style={{width:100,height:90}}/>
              ...
              <td className="py-3 px-6 text-center">
                ...
                <div className="flex item-center justify-center">               
                  <Link onClick={()=>getIdevent(item._id)} className="w-4 mr-2 transform hover:text-yellow-500 hover:scale-110 ">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                    </svg>
                  </Link>
                  <Link onClick={()=>deleteevent(item._id)} className="w-4 mr-2 transform hover:text-red-500 hover:scale-110">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
