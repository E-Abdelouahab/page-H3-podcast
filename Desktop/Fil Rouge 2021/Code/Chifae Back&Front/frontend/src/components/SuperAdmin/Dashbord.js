...
// get all admin and show it in table
  ...
  useEffect(()=>{
    ...
    axios.get(`http://localhost:3030/superAdmin/getAlldoctor`)
      .then(function (response) {
          
        setdoctor(response.data)
        setdoctorLength(response.data.length)
      }).catch(function (err) {
        console.log(err);
    });
...
useEffect(()=>{
  ...
  axios.get(`http://localhost:3030/superAdmin/getAllassSec`)
    .then(function (response) {
      ...
      setassSec(response.data.length)
                ...
                <ul className="max-h-full p-2 space-y-1 overflow-y-auto divide-y divide-blue-300 neumorphism-shadow">
                  ...
                  <li>
                    <Link to='/doctor' className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                      ...
                      <span className="ml-2 font-medium">Space doctor</span>
                    </Link>
                  </li>
                  {/* Profile link */}
                  <li>
                    <Link to='/assSec' className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                      <span>
                        <svg className="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                      </span>
                      <span className="ml-2 font-medium">Space association Secondaire</span>
                    </Link>
                  </li>
                </ul>
              </nav>
    ...
    <div className="flex flex-wrap -m-4 text-center">
      ...
      <div className="p-4 md:w-2/4 sm:w-1/2 w-full">
        <div className="border-2 border-gray-200 px-4 py-6 rounded-lg">
          ...
          <h2 className="title-font font-medium text-3xl text-gray-900">{doctorLength}</h2>
      ...
      <div className="p-4 md:w-2/4 sm:w-1/2 w-full">
        <div className="border-2 border-gray-200 px-4 py-6 rounded-lg">
          ...
          <h2 className="title-font font-medium text-3xl text-gray-900">{assSec}</h2>
          <p className="leading-relaxed">assSociation Secondaire</p>
        </div>
      </div>
