...
const ConfirmassSec = () => {
  ...
  const [assSec, setassSec] = useState();
...
// get all sellers 
  ...
  useEffect(()=>{
    ...
    axios.get(`http://localhost:3030/superAdmin/getAllassSec`)
      .then(function (response) {
          
        setassSec(response.data)
  ...
  // delete Admin 
  const deleteassSec = (id)=>{
    axios.delete(`http://localhost:3030/superAdmin/deleteassSec/${id}`)
    .then(function (response) {
    ...
    const getIdassSec = (id)=>{
      localStorage.setItem('idasssec',id);
      history.push('/confirmassSec');
            ...
            <div className="flex flex-col h-full pt-12 pb-4 rounded-lg neumorphism-shadow bg-white">
              ...
              <nav className="flex-1 max-h-full p-4 mt-6 overflow-y-hidden">
                <ul className="max-h-full p-2 space-y-1 overflow-y-auto divide-y divide-blue-300 neumorphism-shadow">
                  ...
                  <li>
                    <Link to='/doctor' className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                      <span>
                        <svg className="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                      </span>
                      <span className="ml-2 font-medium">Space Doctors</span>
                    </Link>
                  </li>
                  {/* Profile link */}
                  <li>
                    <Link className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                      ...
                      <span className="ml-2 font-medium">Space assSec</span>
                    </Link>
                  </li>
                </ul>
              </nav>
...
<div className="overflow-x-auto" style={{width:1200}}>
  ...
  <div className=" flex items-center justify-center font-sans " style={{marginTop:150}}>
    ...
    {/* <Link to='/adddoctor' class="bg-blue-300 hover:bg-green-100 text-black font-bold py-2 px-4 rounded-100">Add Admin</Link> */}
            ...
            <tr className="border-b border-gray-200 hover:bg-gray-100">
              ...
              <td className="py-3 px-6 text-center">
                ...
                <div className="flex item-center justify-center">               
                  <Link onClick={()=>getIdassSec(item._id)} className="w-4 mr-2 transform hover:text-yellow-500 hover:scale-110">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                    </svg>
                  </Link>
                  <Link onClick={()=>deleteassSec(item._id)}  className="w-4 mr-2 transform hover:text-red-500 hover:scale-110">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
