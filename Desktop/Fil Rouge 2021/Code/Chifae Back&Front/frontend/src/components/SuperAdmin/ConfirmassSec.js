...
const ConfirmSeller=()=> {
  ...
  const id_assSec=localStorage.getItem('idassSec');
  let idassSec;
  let idassSec2;
...
useEffect(()=>{
  ...
  axios.get(`http://localhost:3030/superAdmin/getassSecById/${id_assSec}`)
...
const handleSubmit = (e) => {
  e.preventDefault();
var id = idassSec.value;
var id2 = idassSec2.value;
const data = {type:id,status:id2};

axios.put(`http://localhost:3030/superAdmin/updateassSec/${id_assSec}`,data)
.then(res => {
  if(res.error){
    return false
  }else{
