...
const  Editdoctor = () => {
    ...
    const id_doctor=localStorage.getItem('iddoctor');

// ---------------------get Doctor to update-----------------------------
useEffect(()=>{

    axios.get(`http://localhost:3030/superAdmin/getdoctorById/${id_doctor}`)
...
axios.put(`http://localhost:3030/superAdmin/updatedoctor/${id_doctor}`,data)
  .then(res => {
    if(res.error){
      return false
    }else{
